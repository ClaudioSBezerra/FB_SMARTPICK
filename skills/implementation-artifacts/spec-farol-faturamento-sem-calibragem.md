...
- Comparação: produto = pendente quando está em `vendas_faturadas` (Farol, últimos 30 dias, `classe_venda` A ou B) e NÃO existe `sp_propostas` com `status='aprovada'` e `aprovado_em` nos últimos 30 dias para o mesmo `(cd_id, codprod)`.
...
|----------|--------------|---------------------------|----------------|
| Happy path | CD com produtos Curva A/B faturados (Farol) nos últimos 30d, alguns sem calibragem aprovada | Lista os produtos pendentes (codprod, classe, qtd faturada) | N/A |
...
**Acceptance Criteria:**
- Given um CD com produtos Curva A/B faturados nos últimos 30 dias no Farol, when nenhum tem `sp_propostas` aprovada correspondente no mesmo período, then o painel lista esses produtos como pendentes.
