...
- 2026-08-31 (pós-deploy, usuário — bug na coluna Sazonalidade): usuário reportou que a coluna vinha vazia para TODOS os produtos. Primeira hipótese (**parcialmente equivocada**, ver correção abaixo): o cruzamento com o índice sazonal por Seção usava `cod_depto`/`cod_sec` vindos de `vendas_faturadas` via `/api/farol/produtos-faturados` (Farol) — no código de import do Farol (`farol_v2_import.go`) esses campos pertencem a um layout novo (jul/2026) ainda opcional lá ("ficam vazias no CSV antigo"), o que levou a crer que chegariam sempre em branco. Corrigido trocando a fonte: `carregarClassificacaoCurva` (`sp_enderecos`, já usada pra Curva A/B) passou a também carregar `codepto`/`codsec` do próprio SmartPick — usuário confirmou que é a mesma numeração WinThor do Farol. `coletarFaturamentoInterno` usa agora `classificacaoProduto.codDepto/codSec` em vez de `FarolProdutoFaturado.CodDepto/CodSec` pra montar a chave de match contra `GetSazonalidadeSecao`.
  **Causa raiz real** (usuário contestou a hipótese acima — "temos índices por departamento e seção nas análises por indústria", e trouxe prints confirmando que tanto `sp_enderecos` quanto `vendas_faturadas` têm os códigos corretos e batendo entre si, ex.: 35/3510 = LIMPEZA/LIMPADORES nos dois sistemas): não era dado faltando, era **timeout**. Query direta em produção (`sp_enderecos` do CD 01 FILIAL 11) confirmou `codepto`/`codsec` presentes e corretos; logs do backend em produção mostraram `GetSazonalidadeSecao` falhando toda vez com "context deadline exceeded" nos 20s do `farolHTTPClient` compartilhado — a consulta de sazonalidade agrega o ano inteiro (2025) por Departamento×Seção×Mês, bem mais pesada que a janela curta de `produtos-faturados` que usa o mesmo cliente. Corrigido com um `http.Client` próprio de 60s só pra essa chamada (`farolSazonalidadeHTTPClient`, `farol_client.go`), sem alterar o timeout das demais chamadas ao Farol. A troca de fonte pro `sp_enderecos` (parágrafo acima) continua válida — é uma fonte mais robusta, só não era a causa raiz deste sintoma.
