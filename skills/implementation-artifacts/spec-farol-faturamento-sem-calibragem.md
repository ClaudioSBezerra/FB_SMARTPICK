...
- 2026-08-31 (pós-deploy, usuário — bug na coluna Sazonalidade): usuário reportou que a coluna vinha vazia para TODOS os produtos. Causa raiz: o cruzamento com o índice sazonal por Seção usava `cod_depto`/`cod_sec` vindos de `vendas_faturadas` via `/api/farol/produtos-faturados` (Farol) — mas no código do Farol (`farol_v2_import.go`) esses campos pertencem a um layout de importação novo (jul/2026) ainda opcional lá ("ficam vazias no CSV antigo"), chegando sempre em branco em produção. Corrigido trocando a fonte: `carregarClassificacaoCurva` (`sp_enderecos`, já usada pra Curva A/B) passou a também carregar `codepto`/`codsec` do próprio SmartPick — usuário confirmou que é a mesma numeração WinThor do Farol. `coletarFaturamentoInterno` usa agora `classificacaoProduto.codDepto/codSec` em vez de `FarolProdutoFaturado.CodDepto/CodSec` pra montar a chave de match contra `GetSazonalidadeSecao`. Não depende mais do Farol adotar o layout novo.
