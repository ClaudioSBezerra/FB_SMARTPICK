		...
		query := `
			WITH enderecos_agrupados AS (
				...
				WHERE e.capacidade IS NOT NULL
				  -- Calibração: inclui dados não-marcados (tipo_rel NULL de cargas
				  -- antigas ou de arquivos sem a coluna TIPO_REL). Exclui só REALOCACAO.
				  AND (e.tipo_rel = 'CALIBRACAO' OR e.tipo_rel IS NULL)
				...
				SELECT
					...
					(
						...
						WHERE p.codprod    = ea.codprod
						  ...
						  AND p.empresa_id = $1
						  AND (p.tipo_rel = 'CALIBRACAO' OR p.tipo_rel IS NULL)
